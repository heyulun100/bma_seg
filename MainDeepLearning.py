...
if run_the_2D_model and added_lr:
    from DL_Template.Run2DModel import run_2d_model
    run_2d_model(batch_size=batch_size)

add_metrics_to_excel = False
if add_metrics_to_excel:
    from DL_Template.AddMetricsToExcel import add_metrics_to_excel
    add_metrics_to_excel()
    xxx = 1

review_models_via_cv = False
if review_models_via_cv:
    from DL_Template.Find_Mean_Std_Across_CV_Groups import \
...
if view_gradients:
    ...
    import numpy as np
    from DL_Template.ReturnGenerators import return_generators, plot_scroll_Image
    from DL_Template.ReturnModels import return_model
    from DL_Template.ReturnCosineLoss import CosineLoss
    from Base_Deeplearning_Code.Visualizing_Model.Visualize_Model import ModelVisualizationClass
...
if evaluate_model:
    from tensorflow.keras.models import load_model
    import numpy as np
    from DL_Template.ReturnGenerators import return_generators
    from DL_Template.ReturnModels import return_model
    from DL_Template.ReturnCosineLoss import CosineLoss
    model_path = r'H:\Deeplearning_Recurrence_Work\Nifti_Exports\Records\Models\Model_Index_140\final_model.h5'
    model = load_model(model_path, custom_objects={'CosineLoss': CosineLoss})
